...
- **Configurador e CRUD Completo de Planos (`/admin/plans`):** Permite **Criar Novos Planos** e **Excluir Planos** com confirmação (`ConfirmModal`). Cada cartão de plano renderiza de forma sincronizada: (1) **Benefícios Textuais (Landing Page & Checkout)** com checkmarks verdes (`featuresList`) e (2) **Permissões de Telas do App (Segurança & Acesso)** com toggles de liberação (`allowedFeatures`). O campo visual de Créditos IA foi removido para simplificar a oferta e evitar confusões para os usuários.
