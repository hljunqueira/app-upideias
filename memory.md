...
- **Papel da Tela de Contas no Admin (`/admin/accounts`):** O painel `/admin/accounts` atua exclusivamente para monitoramento de saúde de tokens, exibição de detalhes da Phyllo, execução de sincronizações globais (`mockSyncInstagramMetrics`) e revogação de permissões com `ConfirmModal`.
