...
- **Proibição Absoluta de Dados Mocks/Hardcoded (Role User & Admin):** Todas as páginas do Assinante (`/app/*`) e do Admin (`/admin/*`) devem obrigatoriamente consumir e persistir dados reais no banco de dados PostgreSQL via Supabase (`plans`, `courses`, `lessons`, `user_lesson_progress`, `content_ideas`, `content_calendar`, `content_approvals`, `content_library`, `subscriptions`, `notification_preferences`). É proibido manter arrays fictícios estáticos, multiplicadores sintéticos em gráficos ou marcas "Demo".
- **Padronização de Modais de Confirmação (`ConfirmModal`):** Qualquer ação destrutiva ou de aviso de limite (ex: deleção de agendamentos, remoção de itens de biblioteca, aviso de limite de clientes no Plano Agência) deve utilizar obrigatoriamente o componente `ConfirmModal` em substituição aos `alert()` / `confirm()` nativos do navegador.
