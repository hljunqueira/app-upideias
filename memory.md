...
- **Trilhas e Cursos Dinâmicos no UP Creator (`CreatorShowcase.tsx`):** Os botões de filtro de trilhas e os cards de cursos na Landing Page consomem dados dinâmicos do Supabase (`learning_trails` e `courses`) e das stores via `fetchTrailsFromDb()` e `fetchCoursesFromDb()`. É estritamente proibido utilizar arrays fixos de categorias hardcoded (ex: `tracks = [...]`).
