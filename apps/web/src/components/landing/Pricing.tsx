...
function SpotlightCard({ p, i, annual }: { p: (typeof plans)[0]; i: number; annual: boolean }) {
  ...
  return (
    <motion.div
      ...
      className={`relative flex flex-col rounded-3xl border p-8 backdrop-blur-xl transition-all duration-300 hover:-translate-y-2 overflow-visible ${
        p.featured
          ? "bg-upCard border-upPink/40 lg:scale-[1.05] shadow-[0_0_60px_rgba(255,83,104,0.18)] mt-4 lg:mt-0"
          : "bg-upCard/60 border-upBorder hover:border-upPink/40"
      }`}
      data-testid={`pricing-card-${p.slug}`}
    >
      {p.featured && <span className="conic-border pointer-events-none opacity-70 rounded-3xl" />}
      ...
      <div className="relative pt-2">
        {p.featured && (
          <span className="absolute -top-6 left-1/2 -translate-x-1/2 bg-upPink text-white text-[11px] font-bold uppercase tracking-wider px-3.5 py-1 rounded-full shadow-[0_0_20px_rgba(255,83,104,0.5)] z-20 whitespace-nowrap">
            Mais popular
          </span>
        )}
        <h3 className="font-display text-2xl font-bold text-white mt-3">{p.name}</h3>
