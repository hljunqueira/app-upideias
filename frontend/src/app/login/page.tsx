...
import { apiLogin, loginWithGoogle } from "@up-analytics/lib";
...
export default function Login() {
  ...
  const doLogin = async (mail: string, pass: string, who: string) => {
    setError("");
    setLoading(who);
    try {
      const user = await apiLogin(mail, pass);
      router.push(user?.role === "admin" ? "/admin" : "/app/dashboard");
    } catch (err: any) {
    ...
    <AuthShell script="bem-vindo de volta —" headline={"ENTRE NO\nMUNDO UP."}>
      <div className="lg:hidden text-center mb-8">
        <Link href="/" className="inline-flex items-center gap-2">
          <img src="/UP-Logo-removebg-preview.png" alt="UP Ideias" className="h-10 w-auto" />
          <span className="font-display text-lg font-bold text-white">UP <span className="text-upPink">IDEIAS</span></span>
        </Link>
      </div>
      ...
      {error && (
        <div data-testid="login-error" className="mt-6 px-4 py-3 bg-upPink/10 border border-upPink/30 rounded-xl text-xs text-upPink font-semibold">
          {error}
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-5 mt-8">
        ...
        >
          {loading === "form" ? "Entrando..." : "Entrar no Painel"}
      ...
      {/* Demo quick access */}
      <div className="mt-8 bg-upCard/40 border border-upBorder/60 rounded-2xl p-4">
        <p className="text-[10px] text-upGray uppercase tracking-widest font-bold mb-3">Acesso rápido (demo)</p>
        <div className="grid grid-cols-2 gap-3">
          <button
            type="button"
            data-testid="demo-user-login-btn"
            disabled={loading !== null}
            onClick={() => doLogin("creator@upideias.com", "Creator@2026", "user")}
            className="flex items-center justify-center gap-2 py-3 rounded-xl border border-upBorder text-upLightGray hover:text-white hover:border-upPink/60 hover:bg-upPink/10 transition-all text-xs font-semibold disabled:opacity-60"
          >
            <Star className="w-3.5 h-3.5 text-upPink" />
            {loading === "user" ? "Entrando..." : "Assinante"}
          </button>
          <button
            type="button"
            data-testid="demo-admin-login-btn"
            disabled={loading !== null}
            onClick={() => doLogin("admin@upideias.com", "Admin@2026", "admin")}
            className="flex items-center justify-center gap-2 py-3 rounded-xl border border-upBorder text-upLightGray hover:text-white hover:border-upPink/60 hover:bg-upPink/10 transition-all text-xs font-semibold disabled:opacity-60"
          >
            <Shield className="w-3.5 h-3.5 text-upPink" />
            {loading === "admin" ? "Entrando..." : "Admin"}
          </button>
        </div>
      </div>
